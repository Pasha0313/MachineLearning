...
class Config:
    ...
    MODEL_PATH: str = os.path.join(BASE_DIR, "models", "mistral-7b-instruct-v0.1.Q4_K_M.gguf")
...
class DataValidator:
    ...
    def validate_hs_code_format(code: str) -> bool:
        ...
        if not code or pd.isna(code):
            return False
        #Uses a regular expression to remove all non-digit characters.
        #replaces those characters with an empty string
    ...
    def clean_data(df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Converts all string values in the "sku_code" column to uppercase.
        ...
        # Replaces any NaN (missing) values in that column with.
...
class LLMProcessor:
    ...
    def initialize_llm(self, model_path: str):
        ...
        try:
            self.llm = AutoModelForCausalLM.from_pretrained(
                model_path,
                model_type="mistral",
                gpu_layers=0,  # Use 0 for CPU, >0 for GPU
                lib="avx2"  # Use AVX2 for better performance 
                #ses AVX2 vector instructions for efficient computation. Most modern Intel/AMD CPUs support this.
            )
    ...
    def create_enhanced_prompt(self, row: pd.Series) -> str:
        ...
        # Generate deterministic metadata based on SKU # This gives you a repeatable numeric representation of the SKU.
        # Converts the string sku into bytes — required by the hashlib module.
        # Computes the MD5 hash of the encoded SKU. Produces a 32-character hexadecimal string (128 bits). (1)
        sku_hash = hashlib.md5(sku.encode()).hexdigest()
        hash_int = int(sku_hash[:8], 16)
        
        # Use hash to select category and product with more variety
        # selects a deterministic category and product based on the sku string using hashing
        # (1) plus This hash_int will always be the same for the same SKU.
        categories = ["electronics", "clothing", "furniture", "tools", "kitchenware", "sports", "automotive", "jewelry"]
        category = categories[hash_int % len(categories)]
        
        products = {
            "electronics": ["smartphone", "laptop", "bluetooth speaker", "digital camera", "tablet", "headphones", "smartwatch"],
            "clothing": ["cotton t-shirt", "denim jeans", "leather jacket", "wool sweater", "running shoes", "formal shirt"],
            "furniture": ["wooden chair", "metal desk", "plastic table", "leather sofa", "bookshelf", "office chair"],
            "tools": ["steel hammer", "electric drill", "screwdriver set", "wrench", "saw", "pliers"],
            "kitchenware": ["ceramic mug", "stainless steel pan", "glass bowl", "plastic container", "wooden spoon"],
            "sports": ["basketball", "tennis racket", "yoga mat", "dumbbells", "bicycle helmet", "running shoes"],
            "automotive": ["car battery", "tire", "brake pad", "oil filter", "spark plug", "windshield wiper"],
            "jewelry": ["gold necklace", "silver ring", "diamond earrings", "pearl bracelet", "crystal pendant"]
        }
        
        product = products[category][hash_int % len(products[category])]
        
        # Add more specific details based on hash
        materials = ["cotton", "plastic", "metal", "wood", "leather", "glass", "ceramic", "synthetic"]
        material = materials[hash_int % len(materials)]
        
        sizes = ["small", "medium", "large", "standard", "compact", "oversized"]
        size = sizes[hash_int % len(sizes)]
        
        # Create more detailed description
        full_description = f"{sku} - {size} {material} {product} for {category}"
        ...
        instruction = (
               ...
               f"Please respond with only the correct 10-digit HS Code.\n"
               f"HS Code:"
                )
    ...
    def process_with_llm(self, prompt: str, retry_count: int = 0) -> Dict:
        """Process prompt with ctransformers"""
        try:
            start_time = time.time()
            
            # Generate response with ctransformers
            # Try different generation parameters for better output
            output = self.llm(
                prompt,
            ...
            self.logger.info(f"Raw LLM output: {repr(output)}")
            
            # If output is empty, try with different parameters
            if not output or output.strip() == "":
                self.logger.warning("Empty output, trying with different parameters...")
                output = self.llm(
                    prompt,
                    max_new_tokens=20,
                    temperature=0.1,
                ...
                self.logger.info(f"Second attempt output: {repr(output)}")
    ...
    def _extract_hs_code(self, text: str) -> Optional[str]:
        """Enhanced HS code extraction: accept dotted codes and normalize to 10 digits"""
        # Accept codes with dots (e.g., 9604.99.9990)
        match = re.search(r"(\d{4}\.\d{2}\.\d{4})", text)
        if match:
            code = match.group(1).replace('.', '')
            return code.zfill(10) if len(code) < 10 else code[:10]
    ...
    def _get_fallback_hs_code(self, category: str) -> str:
        """Get fallback HS code based on product category"""
        fallback_codes = {
            "electronics": "8471300000",  # General electronics
            "clothing": "6109100000",     # General clothing
            "furniture": "9401610000",    # General furniture
            "tools": "8205200000",        # General tools
            "kitchenware": "6912000000"   # General kitchenware
        }
        return fallback_codes.get(category, "9999999999")
    ...
    def run_phase3_enrich(self, df: pd.DataFrame, max_rows: int = None) -> pd.DataFrame:
        ...
        try:
            ...
            for idx, row in enrichment_rows.iterrows():
                ...
                # Log result
                if result["hs_code"]:
                    self.logger.info(f"[SUCCESS] Generated HS Code: {result['hs_code']} (confidence: {result['confidence']:.2f})")
                else:
                    self.logger.warning(f"[FAILED] Failed to generate HS Code for {sku}")
                    # Use fallback HS code based on SKU hash
                    sku_hash = hashlib.md5(str(sku).encode()).hexdigest()
                    hash_int = int(sku_hash[:8], 16)
                    categories = ["electronics", "clothing", "furniture", "tools", "kitchenware"]
                    category = categories[hash_int % len(categories)]
                    fallback_code = self.llm_processor._get_fallback_hs_code(category)
                    self.logger.info(f"[FALLBACK] Using fallback HS Code: {fallback_code} for {sku} (category: {category})")
                    result["hs_code"] = fallback_code
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"[SUCCESS] Pipeline completed successfully. Processed {len(result_df)} rows.")
        
        # Print summary statistics
        print(f"[STATS] High confidence predictions: {(result_df['confidence'] >= config.HIGH_CONFIDENCE_THRESHOLD).sum()}")
        print(f"[STATS] Medium confidence predictions: {((result_df['confidence'] >= config.MEDIUM_CONFIDENCE_THRESHOLD) & (result_df['confidence'] < config.HIGH_CONFIDENCE_THRESHOLD)).sum()}")
        print(f"[STATS] Low confidence predictions: {(result_df['confidence'] < config.MEDIUM_CONFIDENCE_THRESHOLD).sum()}")
        print(f"[STATS] Required reviews: {(result_df['review_status'] == 'required').sum()}")
